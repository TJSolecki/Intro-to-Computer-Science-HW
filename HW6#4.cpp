...
int main()
{
  ifstream myfile("CDList.txt");
  string list[100], line;
  int numUsed = 0;
  if (myfile.is_open())
  {
    ...
    //the following for loop code should switch the author and the title but it doesnt work at least on my computer
    //I spent an hour in office hours trying to get it to work didnt have any luck. Try to run the program with and without it
    for(int i = 0; i < numUsed; i++)
    {
      for(int j = 0; j < list[i].length(); j++)
      {
